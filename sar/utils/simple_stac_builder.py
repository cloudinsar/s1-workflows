...
def generate_catalog(
    stac_root,
    files: Union[str, List[Path], List[List[Path]]] = "*2images*.tif",
    ...
    for file_element in tiff_files:
        ...
        for file in file_element:
            ...
            if not stac["properties"]["datetime"]:
                # only take first date.
            ...
            stac["geometry"]["coordinates"] = coordinates
            latlon_bbox = [
            ...
            stac["assets"][file.name] = gdalinfo_stac

        stac_item_filename = str(Path(str(stac["id"]) + ".json").absolute())
