...
for pol in input_dict["polarization"]:
    ...
    if (primary_date < start_date) or (primary_date > end_date):
        ...
        if len(burst_primary) == 0:
            raise Exception(f"No bursts found for primary_date: {input_dict.get('primary_date')}, burst_id: {input_dict.get('burst_id')}, subswath: {input_dict.get('sub_swath')}")
    ...
    for burst in bursts:
        ...
        _, output = exec_proc_retried(cmd, cwd=repo_directory / "utilities", write_output=False)
